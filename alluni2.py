...
def find_valid_parameters_by_k(n_start, n_end, k_values):
    """Find all (n, k, s) for each k such that W(H) = W(H minus 0) and diam(H) = 1 and W(H) = binom(n, 2)."""
    valid_by_k = defaultdict(list)

    for n in range(n_start, n_end + 1):
        binom_n_2 = n * (n - 1) // 2  # Precompute binomial coefficient
        for k in k_values:
            for s in range(0, k):
                if k >= 3 and 2 * s + k <= n:  # Basic feasibility
                    hyperedges = generate_hyperedges(n, k, s)
                    wiener_full, diameter_full = compute_wiener_index(n, hyperedges)

                    # ✅ Only continue if W(H) = binom(n,2) and diam(H) = 1
                    if wiener_full != binom_n_2 or diameter_full != 1:
                        continue
                    ...
                    if wiener_full == wiener_removed:
                        valid_by_k[k].append((n, k, s))
                        print(f"Valid case for k={k}: n={n}, s={s}")
...
n_start = 120
n_end = 130
k_values = range(60, 100)  # Custom k range

valid_results = find_valid_parameters_by_k(n_start, n_end, k_values)
