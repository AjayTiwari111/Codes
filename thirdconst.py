...
def generate_hyperedges(n, k, s):
    """Construct hyperedges: take 3 vertices, skip s, take k-6 vertices, skip s, take 3 vertices."""
    hyperedges = []
    for i in range(n):
        edge = []
        ...
        # Skip s, then take k - 6 vertices
        start_mid = (i + 3 + s) % n
        for j in range(k - 6):
            edge.append((start_mid + j) % n)

        # Skip another s, then take final 3 vertices
        start_end = (start_mid + (k - 6) + s) % n
        for j in range(3):
...
def find_valid_parameters(n_start, n_end):
    ...
    for n in range(n_start, n_end + 1):
        ...
        for k in range(d, n): #replaced n+1
            for s in range(0, 20):
