...
def generate_hyperedges(n, k, s):
    ...
    for i in range(n):
        ...
        # Include k - 3 consecutive vertices
        for j in range(k - 4):
            hyperedge.append((i + j) % n)

        # Include 1 after skipping s
        hyperedge.append((i + k - 4 + s) % n)

        # Include 1 after skipping another s
        hyperedge.append((i + k - 4 + 2 * s + 1) % n)

        # Include 1 after another s skip
        hyperedge.append((i + k - 4 + 3 * s + 2) % n)

        # Include 1 after another s skip
        hyperedge.append((i + k - 4 + 4 * s + 3) % n)
...
# Example usage:
n_start = 10  # Starting value of n
n_end = 40   # Ending value of n
