...
def generate_hyperedges(n, k, s):
    hyperedges = []
    for i in range(n):
        # Include k-2 consecutive vertices starting from i
        hyperedge = [(i + j) % n for j in range(k - 2)]

        # Skip s vertices, include next one
        first_extra = (i + k - 2 + s) % n
        hyperedge.append(first_extra)

        # Skip another s vertices, include next one
        second_extra = (i + k - 2 + 2 * s + 1) % n
        hyperedge.append(second_extra)
