...
def generate_hyperedges(n, k, s):
    ...
    """
    if k < 10:
        raise ValueError("k must be at least 10 to accommodate two blocks of 5 vertices.")
    ...
    for i in range(n):
        ...
        # Skip s, then take k - 10 vertices
        start_mid = (i + 5 + s) % n
        for j in range(k - 10):
            edge.append((start_mid + j) % n)

        # Skip another s, then take final 5 vertices
        start_end = (start_mid + (k - 10) + s) % n
        for j in range(5):
    ...
    for n in range(n_start, n_end + 1):
        ...
        for k in range(d, n+1): #replaced n+1
            for s in range(12, 25):
                if k >= 5 and 2 * s + k <= n:  # Basic feasibility condition
